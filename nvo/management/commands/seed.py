...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.edit_create_options = [
            ("add_", "Can add "),
            ("change_", "Can change "),
            ("view_", "Can view "),
        ]
        self.edit_create_delete_options = [
            ("add_", "Can add "),
            ("change_", "Can change "),
            ("view_", "Can view "),
            ("delete_", "Can delete "),
        ...
        models.DocumentCategory.objects.get_or_create(name="vsebinsko poročilo")
        models.DocumentCategory.objects.get_or_create(name="finančno poročilo")
        ...
        permissions = self.get_permissions(
            "organizationfinancialyear", ct, self.edit_options
        )
        nvo_group.permissions.add(*permissions)
        ...
        permissions = self.get_permissions(
            "document", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)
        ...
        permissions = self.get_permissions(
            "project", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)

        ct = ContentType.objects.get_for_model(models.Financer)
        permissions = self.get_permissions(
            "financer", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)

        ct = ContentType.objects.get_for_model(models.CoFinancer)
        permissions = self.get_permissions(
            "cofinancer", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)

        ct = ContentType.objects.get_for_model(models.Partner)
        permissions = self.get_permissions(
            "partner", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)

        ct = ContentType.objects.get_for_model(models.Donator)
        permissions = self.get_permissions(
            "donator", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)
        ...
        permissions = self.get_permissions(
            "personaldonator", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)

        ct = ContentType.objects.get_for_model(models.OrganiaztionDonator)
        permissions = self.get_permissions(
            "organiaztiondonator", ct, self.edit_create_delete_options
        )
        nvo_group.permissions.add(*permissions)
