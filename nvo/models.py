...
def document_size_validator(
    ...
    if value.size > limit:
        raise ValidationError(
            "Datoteka je prevelika. Največja možna velikost je 10 MB."
        )
...
class Organization(models.Model):
    ...
    tax_number = models.CharField(
        max_length=10, null=True, verbose_name=_("TAX number")
    )
    registration_number = models.TextField(
        null=True, verbose_name=_("Registration number")
    )
    email = models.EmailField(null=True, verbose_name=_("Email"))
    phone_number = models.CharField(
        max_length=13, null=True, blank=True, verbose_name=_("Phone number")
    )
    ...
    is_for_the_public_good = models.TextField(
        default=None,
        null=True,
        blank=True,
        verbose_name=_("Is organization for public good"),
    )

    financial_years = models.ManyToManyField(
        FinancialYear,
        related_name="organizations",
        through="OrganizationFinancialYear",
        verbose_name=_("Financial years"),
    )
    ...
    def get_years(self):
        return (
            self.financial_years.filter(organiaztion_through__is_active=True)
...
class Document(models.Model):
    ...
    category = models.ForeignKey(
        "DocumentCategory", on_delete=models.CASCADE, verbose_name=_("Category")
    )
    organization = models.ForeignKey(
        "Organization",
        on_delete=models.CASCADE,
...
class People(models.Model):
    ...
    number_of_non_binary = models.IntegerField(
        null=True, verbose_name=_("Number of non binary")
    )
...
class PaymentRatio(models.Model):
    ...
    highest_absolute_salary = models.FloatField(
        null=True, blank=True, verbose_name=_("Highest absolute salary")
    )
    highest_salary = models.FloatField(
        null=True, blank=True, verbose_name=_("Highest salary")
    )
    lowest_salary = 1
    average_salary = 1
    ...
    def get_statistics(self):
        return {
            ...
            "highest_absolute": (
                self.highest_absolute_salary if self.highest_absolute_salary else 1
            ),
...
class Finance(models.Model):
    ...
    amount_voluntary_work = models.DecimalField(
        decimal_places=2,
        max_digits=10,
        null=True,
        blank=True,
        verbose_name=_("Amount of voluntary work"),
    )
    payments_project_partners = models.DecimalField(
        decimal_places=2,
        max_digits=10,
        null=True,
        blank=True,
        verbose_name=_("Payments to projects partners"),
    )
    payment_state_budget = models.DecimalField(
        decimal_places=2,
        max_digits=10,
        null=True,
        blank=True,
        verbose_name=_("Payment to the state budget"),
    )
    acquired_state_budget = models.DecimalField(
        decimal_places=2,
        max_digits=10,
        null=True,
        blank=True,
        verbose_name=_("Difference payment state budget"),
    )
...
class FinancialCategory(MPTTModel):
    ...
    additional_name = models.CharField(
        max_length=256, null=True, blank=True, verbose_name=_("Additional name")
    )
    ...
    allow_additional_name = models.BooleanField(
        default=False, verbose_name=_("Allow additional name")
    )
...
class Project(models.Model):
    ...
    value = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Total value")
    )
    self_money = models.DecimalField(
        null=True,
        blank=True,
        max_digits=10,
        decimal_places=2,
        verbose_name=_("Self money"),
    )
    ...
    def icons(self):
        financer_icons = [
            financer.logo for financer in self.financers.all() if financer.logo
        ]
        co_financer_icons = [
            co_financer.logo
            for co_financer in self.cofinancers.all()
            if co_financer.logo
        ]
    ...
    def duration(self):
        r = relativedelta.relativedelta(
            self.end_date + timedelta(days=1), self.start_date
        )
        return {
            "days": r.days,
...
class Financer(models.Model):
    project = models.ForeignKey(
        "Project", on_delete=models.CASCADE, related_name="financers"
    )
    name = models.TextField(verbose_name=_("Name"))
    link = models.URLField(
        null=True, blank=True, verbose_name="Povezava do spletnega mesta"
    )
    logo = models.ImageField(
        null=True,
        blank=True,
...
class CoFinancer(models.Model):
    project = models.ForeignKey(
        "Project", on_delete=models.CASCADE, related_name="cofinancers"
    )
    name = models.TextField(verbose_name=_("Name"))
...
class Partner(models.Model):
    project = models.ForeignKey(
        "Project", on_delete=models.CASCADE, related_name="partners"
    )
    name = models.TextField(verbose_name=_("Name"))
...
class Donator(models.Model):
    project = models.ForeignKey(
        "Project", on_delete=models.CASCADE, related_name="donators"
    )
    name = models.TextField(verbose_name=_("Name"))
...
class Donations(models.Model):
    ...
    personal_donations_amount = models.DecimalField(
        null=True,
        blank=True,
        max_digits=10,
        decimal_places=2,
        verbose_name=_("Personal donation amount"),
    )
    number_of_personal_donations = models.IntegerField(
        null=True, blank=True, verbose_name=_("Number of personal donations")
    )
    organization_donations_amount = models.DecimalField(
        null=True,
        blank=True,
        max_digits=10,
        decimal_places=2,
        verbose_name=_("Organization donations amount"),
    )
    number_of_organization_donations = models.IntegerField(
        null=True, blank=True, verbose_name=_("Number od organization donations")
    )
    one_percent_income_tax = models.DecimalField(
        null=True,
        blank=True,
        max_digits=10,
        decimal_places=2,
        verbose_name=_("1 percent income tax"),
    )
    purpose_of_donations = models.TextField(
        null=True, blank=True, verbose_name=_("Donation purpose")
    )
...
class PersonalDonator(models.Model):
    project = models.ForeignKey(
        "Donations", on_delete=models.CASCADE, related_name="personal_donators"
    )
    name = models.TextField(verbose_name=_("Name"))
...
class OrganiaztionDonator(models.Model):
    project = models.ForeignKey(
        "Donations", on_delete=models.CASCADE, related_name="organiaztion_donators"
    )
    name = models.TextField(verbose_name=_("Name"))
    amount = models.IntegerField(default=0, verbose_name=_("Amount"))
...
class Embed(models.Model):
    ...
    page_of_embed_url = models.URLField(
        null=True, blank=True, verbose_name=_("Page where iframe is embeded")
    )

    def __str__(self):
        return f"Objava podatkov za organizacijo {self.organization}"
